App = {
  ...
  initWeb3: function () {
    ...
    if (typeof web3 !== 'undefined') {
      ...
      App.web3Provider = new Web3(new Web3.providers.HttpProvider('https://rinkeby.infura.io/v3/62af0da84c4548708c454cd8a3ac2099'));
    }
  ...
  bindEvents: function () {
    ...
    $(document).on('click', '.how-to-create', (function () {
      App.showHowToCreate();
    }));
    $(document).on('click', '.how-to-claim', (function () {
      App.showHowToClaim();
    }));
    $(document).on('click', '.go-back', (function () {
      App.showMain();
    }));
    $(document).on('click', '.how-to-edit', (function () { 
      App.showHowToEdit();
    }));
    $(document).on('click', '.how-to-fees', (function () { 
      App.showHowToFees();
    }));
    $(document).on('click', '.privacy-policy', (function () { 
      App.showPrivacyPolicy();
    }));
    $(document).on('click', '.how-to-work', (function () { 
      App.showWork();
    }));
    $(document).on('click', '.dev-log', (function () { 
      App.showDevlog();
    }));
    ...
    $(document).on('click', '.unpause', (function () { 
      App.ownerUnpause();
    }));
    $(document).on('click', '.owner-close', (function () { 
      App.ownerClose($("#owner-close").val());
    }));
    $(document).on('click', '.update-price', (function () { 
      App.ownerUpdatePrice($("#updated-price").val());
    }));
  ...
  eventListeners: function () {
    ...
    App.contracts.TopicshareBounty.deployed().then(function (instance) {
      ...
      BountyBought.watch(function (error, result) {
        if (!error) {
          App.showNormalMessage("Bought " + result.args._numOfCalls + " campaign succsesfully");
  ...
  buyCalls: function () {
    App.showNormalMessage("Starting campaign...");
    var topicshareBountyInstance;
    App.contracts.TopicshareBounty.deployed().then(function (instance) {
      topicshareBountyInstance = instance;
  ...
  ownerPause: function(){
    ...
    return App.contracts.TopicshareBounty.deployed().then(function (instance) {
      topicshareBountyInstance = instance;
      return topicshareBountyInstance.pause();
    }).then(function (result) {
      console.log("Contract paused");
    });
  },
  ownerUnpause: function(){
    var topicshareBountyInstance;
    return App.contracts.TopicshareBounty.deployed().then(function (instance) {
      topicshareBountyInstance = instance;
      return topicshareBountyInstance.unpause();
    }).then(function (result) {
  ...
  ownerClose: function(){
    var topicshareBountyInstance;
    var id = $("#owner-close").val();
    return App.contracts.TopicshareBounty.deployed().then(function (instance) {
      topicshareBountyInstance = instance;
      
      return topicshareBountyInstance.closeBounty(id);
    }).then(function (result) {
      console.log("Bounty Id: " + id + " closed");
    });
  },
  ownerUpdatePrice: function(){
    var topicshareBountyInstance;
    var amount;
    return App.contracts.TopicshareBounty.deployed().then(function (instance) {
      topicshareBountyInstance = instance;
      amount = $("#updated-price").val();
      var price = web3.toWei(amount, 'ether');
      return topicshareBountyInstance.setCallCost(price);
    }).then(function (result) {
      console.log("Fee set to: " + amount + " ETH");
    });
  },
  ownerShowStats: function () {
    var active=0;
    var topicshareBountyInstance;
    ...
    $(".status").text('Total campaigns: ' + App.numOfBounties + ' Active: ' + active + ' Closed: ' + (App.numOfBounties - active));
    ...
    if (tweetUrl.includes("https://twitter.com/")) {
      ...
      App.contracts.TopicshareBounty.deployed().then(function (instance) {
        ...
        if (result == "") {
          ...
          App.showNormalMessage("Checking tweet... (" + count + ")");
  ...
  checkFollowOracle: function (count) {
    ...
    if (tweetUrl.includes("https://twitter.com/")) {
      ...
      App.contracts.TopicshareBounty.deployed().then(function (instance) {
        ...
        if (result == "") {
          // Show loading message
          App.showNormalMessage("Checking followers... (" + count + ")");
          // Recursively call the function after 1 second
          setTimeout(function () {
